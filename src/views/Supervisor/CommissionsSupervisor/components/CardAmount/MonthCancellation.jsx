...
CardProduction.propTypes = {
  className: PropTypes.string
};

const PERCENT = 2.6;

function CardProduction({ className, ...other }) {
  ...
  return (
    <Card className={clsx(classes.root, className)} {...other}>
      {statusCurrentList.data && (
        <Box sx={{ flexGrow: 1 }}>
          <Typography variant="subtitle2">Mes Cancelado</Typography>
          ...
          <Typography variant="h3">
            S/. {fCurrency(statusCurrentList.data.month_cancellation)}
          </Typography>
        </Box>
      )}
      <ReactApexChart
