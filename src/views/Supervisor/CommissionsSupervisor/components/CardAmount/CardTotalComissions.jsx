...
import { fCurrency, fPercent, fNumber } from 'src/utils/formatNumber';
...
CardOutstanding.propTypes = {
  className: PropTypes.string
};

const PERCENT = 0.15;

function CardOutstanding({ className, ...other }) {
  const classes = useStyles();
  const theme = useTheme();

  const { statusCurrentList } = useSelector((state) => state.supervisor);
  ...
  return (
    <Card className={clsx(classes.root, className)} {...other}>
      {statusCurrentList.data && (
        <Box sx={{ flexGrow: 1 }}>
          <Typography variant="subtitle2">Total Comisión</Typography>
          ...
          <Typography variant="h3">
            {'S/'}
            {fCurrency(statusCurrentList.data.total_commission)}
          </Typography>
        </Box>
      )}
